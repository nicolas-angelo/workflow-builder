...
const components: Partial<Components> = {
  ...
  p: ({
    ...
    <p className="not-first:mt-4 break-all leading-6" {...props}>
      {children}
    </p>
  ),
  ...
  a: ({
    ...
    <a
      className="whitespace-pre-wrap break-all font-medium underline underline-offset-4"
      rel="noreferrer"
      target="_blank"
      {...props}
    >
  ...
  code: ({ children, node, className, ...props }) => {
    ...
    return (
      <code
        className={cn(
          'break-all rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm',
          className
        )}
        {...props}
      >
